...
class ResultHelper
{
     ...
     * @param boolean $raw_msg 是否按照$msg原样输出结果描述，如果为false将会在标准库寻找对应的code并拼接至结果描述中
    ...
    {
        $errcode = $ex->getCode();
        return static::generate($errcode === 0 ? static::ERROR_CODE : $errcode, $ex->getMessage(),
            null, true);
    }
     ...
     * @param boolean $raw_message 是否按照$msg原样输出结果描述，如果为false将会在标准库寻找对应的code并拼接至结果描述中
     * @return string 结果信息
     */
    ...
    public static function throwExceptionOnResultError(array $result)
    {
        ...
        } else if ($result[static::CODE_FIELD] !== static::SUCCESS_CODE) {
